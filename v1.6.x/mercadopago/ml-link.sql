...
CREATE VIEW `vw_packs` AS select 'MLA629561085' AS `container_ref`,'MLA651644908,MLA625023993' AS `content_ref` union all select 'MLA662068311' AS `MLA662068311`,'MLA605532039,MLA609698341' AS `MLA605532039,MLA609698341`;

CREATE PROCEDURE `ps_update_packstock`(IN `pack_reference` VARCHAR(15), IN `pack_reference` VARCHAR(15))
    LANGUAGE SQL
    NOT DETERMINISTIC
    MODIFIES SQL DATA
...
select psks_product_attribute_combination.id_product_attribute AS id_product_attribute, group_concat(psks_product_attribute_combination.id_attribute) AS ids_attributes, psks_stock.usable_quantity AS usable_quantity
from psks_stock
...
CREATE FUNCTION `ps_packcontent_updated`(`id_product` INT, `id_product` INT)
    RETURNS int(11)
    LANGUAGE SQL
    NOT DETERMINISTIC
    CONTAINS SQL
